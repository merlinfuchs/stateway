...
var _ Batcher = (*InMemoryBatcher)(nil)

// InMemoryBatcher batches entity changes and commits them to the database
...
type InMemoryBatcher struct {
	...
	config  InMemoryBatcherConfig
}

// Config holds the configuration for the entity change batcher.
type InMemoryBatcherConfig struct {
...
func NewInMemoryBatcher(store store.EntityChangeStore, config InMemoryBatcherConfig) *InMemoryBatcher {
	// Set defaults
	if config.MaxSize == 0 {
		config.MaxSize = 1000
	}
...
func (b *InMemoryBatcher) Start(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to start entity change batcher: %w", err)
	}
...
func (b *InMemoryBatcher) Push(ctx context.Context, change model.EntityChange) error {
	if b.batcher == nil {
...
func (b *InMemoryBatcher) Done() <-chan struct{} {
	if b.batcher == nil {
