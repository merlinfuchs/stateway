...
const deleteGuildStickers = `-- name: DeleteGuildStickers :exec
DELETE FROM cache.stickers WHERE app_id = $1 AND guild_id = $2
`

type DeleteGuildStickersParams struct {
	AppID   int64
	GuildID int64
}

func (q *Queries) DeleteGuildStickers(ctx context.Context, arg DeleteGuildStickersParams) error {
	_, err := q.db.Exec(ctx, deleteGuildStickers, arg.AppID, arg.GuildID)
	return err
}
