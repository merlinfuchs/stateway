...
const deleteGuildEmojis = `-- name: DeleteGuildEmojis :exec
DELETE FROM cache.emojis WHERE app_id = $1 AND guild_id = $2
`

type DeleteGuildEmojisParams struct {
	AppID   int64
	GuildID int64
}

func (q *Queries) DeleteGuildEmojis(ctx context.Context, arg DeleteGuildEmojisParams) error {
	_, err := q.db.Exec(ctx, deleteGuildEmojis, arg.AppID, arg.GuildID)
	return err
}
